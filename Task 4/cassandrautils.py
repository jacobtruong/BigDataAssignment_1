...
mimesistable = os.getenv("mimesis.table", "mimesisdata")
...
MIMESIS_TABLE = os.environ.get("MIMESIS_TABLE") if os.environ.get("MIMESIS_TABLE") else 'mimesis'
...
def saveFakerDf(dfrecords):
    ...
    cqlsentence = "INSERT INTO " + fakertable + " (name, spousename, phone, address, email, ssn, creditcardnumber, creditcardexp, creditcardsecuritycode, creditcardprovider) \
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
    ...
    batches = []
    for idx, val in dfrecords.iterrows():
        batch.add(insert, (val['name'], val['spousename'], val['phone'], val['address'], val['email'], val['ssn'], val['creditcardnumber'], val['creditcardexp'], val['creditcardsecuritycode'], val['creditcardprovider']))
        counter += 1
        if counter >= 100:
            print('inserting ' + str(counter) + ' records')
            totalcount += counter
...
def saveMimesisDf(dfrecords):
    ...
    cqlsentence = "INSERT INTO " + mimesistable + " (name, spousename, phone, age, nationality, email, education, bloodtype, height, weight ) \
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
    ...
    batches = []
    for idx, val in dfrecords.iterrows():
        batch.add(insert, (val['name'], val['spousename'], val['phone'], val['age'], val['nationality'], val['email'], val['education'], val['bloodtype'], val['height'], val['weight']))
        counter += 1
        if counter >= 100:
            print('inserting ' + str(counter) + ' records')
            totalcount += counter
...
def loadDF(targetfile, target):
    if target == 'weather':
        ...
        colsnames = ['name', 'spousename', 'phone', 'address', 'email', 'ssn', 'creditcardnumber', 'creditcardexp', 'creditcardsecuritycode', 'creditcardprovider']
        dfData = pd.read_csv(targetfile, header=None,
                             parse_dates=True, names=colsnames)
        saveFakerDf(dfData)
    elif target == 'mimesis':
        colsnames = ['name', 'spousename', 'phone', 'age', 'nationality', 'email', 'education', 'bloodtype', 'height', 'weight']
        dfData = pd.read_csv(targetfile, header=None,
                             parse_dates=True, names=colsnames)
        saveMimesisDf(dfData)
...
def getDF(source_table):
    ...
    if source_table not in (WEATHER_TABLE, TWITTER_TABLE, FAKER_TABLE, MIMESIS_TABLE):
        return None
